...
// 휴무 날짜는 일반예약과 동일한 휠(오늘+21일) 선택으로 통일(v1.14) → 휠 범위 내 날짜를 쓴다.
...
test('휴가/반차 1단계 승인: 매니저 상신 → 매장매니저관리자 승인 종결', async ({ page }) => {
  ...
  // 날짜는 휠에서 선택(클릭 시 해당 항목으로 스크롤·확정)
  await page.getByTestId('dayoff-date-wheel').locator(`[data-value="${DATE}"]`).click()
...
test('가입 2단계 승인: 매장매니저관리자 1차 → 관리자 2차 → 매니저 로그인 성공', async ({ page }) => {
  ...
  // 신청 행 클릭 → 상세 모달 → 모달에서 최종 승인
  await page.getByTestId('manager-approval-row-pending1').click()
  await expect(page.getByTestId('manager-approval-modal')).toBeVisible()
