...
> **문서 버전**: v1.2 (Nuxt 4 스택 전환 반영)
> **작성일**: 2026-06-20
> **작성자**: PM/PL
> **대상 독자**: 주니어 ~ 시니어 프론트엔드 개발자
> **연계 문서**: [`docs/require_v1.md`](./require_v1.md) (요구사항 정의서 v1.1)
...
| **1** | 도메인 타입 & 더미 데이터 레이어 | `app/types/`, `app/data/`, 슬롯·가격 유틸 | 5·10장 | 2.0 | 2.5 |
...
#### 태스크 체크리스트
- [ ] `app/types/enums.ts` — enum/리터럴 유니언 정의 (역할, 차종, 서비스, 슬롯 상태, 예약 상태)
- [ ] `app/types/domain.ts` — 엔티티 인터페이스 정의(require 5.4 표 기준): `User`, `Store`, `Bay`, `Slot`, `Reservation`, `Manager`, `Price`, `Review`
- [ ] `app/data/stores.ts` — 매장 여러 곳 + 베이 `A1~AN` 더미
- [ ] `app/data/managers.ts` — 매장별 매니저 N명(휴무일 포함) 더미
...
- [ ] 매니저 선택 시 해당 매니저 `dayoffDates`를 예약 컨텍스트로 전달(Phase 5 슬롯 비활성에 사용)
...
> **공수: 4.0일** · **선행조건: Phase 4** · **require_v1.md 참조: 5장(슬롯), 6장(예약), 7장(동시성 1단계)**

#### 목표
**날짜 선택 → 베이(행) × 30분(열) 슬롯 그리드**를 렌더하고, **클라이언트 슬롯 잠금 시뮬레이션 + 낙관적 갱신**으로 동시 예약 충돌을 시뮬레이션한다. 차종/서비스 선택 시 **가격 자동 계산**하고 **현장결제 안내**를 표시한다.

#### 태스크 체크리스트
- [ ] `app/stores/reservation.ts` — 슬롯 맵(status 관리), `holdSlot`, `confirmReservation`, `releaseSlot`
- [ ] `app/components/SlotGrid.vue` — 베이 × 시간 그리드, status별 색상/비활성, 클릭 시 HOLDING
- [ ] 날짜 선택 UI (기본: 오늘 이후 날짜)
- [ ] 매니저 휴무일(`dayoffDates`)에 해당하는 날짜/슬롯 비활성 (require 6.1)
- [ ] 동일 시간대 최대 수용 = 베이 수 N 시각화 (require 5.2)
- [ ] 차종/서비스 select → `getPrice()` 호출 → 금액 표시 + "현장결제만 가능" 안내 (require 6.4)
- [ ] **낙관적 갱신**: 슬롯 클릭 즉시 `HOLDING` 표시 → 확정 직전 충돌 감지 → 충돌 시 토스트로 재선택 유도
- [ ] `app/composables/useToast.ts` — 토스트 알림
- [ ] 확장 포인트(2·3단계 서버 검증 교체 지점)에 `// TODO` 주석 명시
...
#### 완료기준 (DoD)
- [ ] 날짜 선택 시 베이 × 30분 그리드가 렌더된다 (베이 수 = 동일 시간대 최대 수용)
- [ ] 슬롯 클릭 시 즉시 HOLDING(시각적 변화)으로 표시된다
- [ ] 매니저 휴무일 슬롯은 비활성으로 표시되어 선택 불가
- [ ] 차종/서비스 선택 시 금액이 자동 계산되고 "현장결제만" 안내가 보인다
- [ ] HOLDING 상태가 아닌 슬롯 확정 시도 시 토스트로 재선택을 유도한다
- [ ] `npm run type-check`, `npm run lint` 통과
...
| Phase 5 | 예약·슬롯 그리드·동시성 1단계 | 5·6·7장 | **FW5** |
...
> **문서 끝.** 본 로드맵은 require_v1.md v1.1을 기준으로 작성되었으며, 1차 FO + 프론트 더미 데이터 단계에 집중합니다.
