...
> **문서 버전**: v1.6 (require_v1.md **v1.7** 정합 — 4역할·2단계 가입 승인·역할별 페이지 분리 반영)
...
> **문서 끝.** 본 로드맵은 require_v1.md v1.1을 기준으로 작성되어 **v1.6**(매니저 3교대 부분 휴무·베이 슬롯 점유 선택불가 규칙 + 예약 3단계 위저드 분할·진행상태 Pinia 스토어·베이 버튼 그리드 단일화 + 일반 회원가입(FW1) Phase 3.1 — 즉시 가입 더미 + **require v1.7 정합: 4역할·2단계 가입 승인·역할별 페이지 분리 표기 반영**)까지 반영했으며, 1차 FO + 프론트 더미 데이터 단계에 집중합니다. 1차 구현 대상은 `USER`(이메일 인증 즉시 활성, 승인 분기 없음)이며, 이메일 인증/SMTP·매니저/관리자 가입·**2단계 가입 승인 UI(M7→S3)**·역할별 BO 페이지(일반매장매니저/매장매니저관리자/관리자)는 2차(ROADMAP_2, require 13.2·2.2) 과제입니다.
